...
// The _hypha_server variable persists from the previous execution
...
try {
    console.log("📤 Calling chatCompletion API using _hypha_server...");
    
    // Use the _hypha_server variable from previous execution
    if (!_hypha_server) {
        throw new Error("_hypha_server is not available. Previous connection may have failed.");
    }
    
    // Call the chatCompletion method directly on _hypha_server
    // The context (including target agent) is automatically provided by HyphaCore
    const chatGenerator = await _hypha_server.chatCompletion(messages);
